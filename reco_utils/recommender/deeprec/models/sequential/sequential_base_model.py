...
class SequentialBaseModel(BaseModel):
    ...
    # def batch_train(self, batch_data_input, train_sess, step):
        """Train the model for a single epoch with mini-batches.

        Args:
            batch_data_input (Batch): A batch of training data.
        ...
        for batch_data_input in file_iterator:
            if batch_data_input:
                ...
                # (_, _, step_loss, step_data_loss, _, _, _, summary) = step_result
                # (_, _, step_loss, step_data_loss, _, _, summary) = step_result
        ...
        return epoch_loss
        # return step_loss, step_data_loss, None
    ...
    def fit(
        ...
        print(eval_info)
        print("best step: {0}".format(self.best_epoch))
        return self
    ...
    def fit_step(
        ...
        best_metric, self.best_step = 0, 0
        ...
        for epoch in range(1, self.hparams.epochs + 1):
            ...
            for batch_data_input in file_iterator:
                if batch_data_input:
                    step_result = self.train(train_sess, batch_data_input)
                    self.step_train(step, step_result)
                    step += 1
                    if step % self.hparams.eval_step == 0:
                        valid_res = self.run_weighted_eval(valid_file, valid_num_ngs)
                        print(
                        ...
                        eval_info.append((step, valid_res))
                        ...
                        if valid_res[eval_metric] > best_metric:
                            ...
                            if early_stop > 0 and step - self.best_step >= early_stop * self.hparams.eval_step:
                        ...
                        if self.hparams.save_model and self.hparams.MODEL_DIR:
                            ...
                            if progress:
                                checkpoint_path = self.saver.save(
                                    sess=train_sess,
                                    save_path=self.hparams.MODEL_DIR + "step_" + str(step),
                                )
            # valid_res = self.run_weighted_eval(valid_file, valid_num_ngs)
            # print(
            #     "eval valid at epoch {0} step {1}: {2}".format(
        ...
        print(eval_info)
        print("best step: {0}".format(self.best_step))
        return self
    ...
    def run_weighted_eval(self, filename, num_ngs, calc_mean_alpha=False, manual_alpha=False):
        ...
        for batch_data_input in self.iterator.load_data_from_file(
            ...
            if batch_data_input:
                ...
                # add filter 0 example
                the_batch_group_preds = np.reshape(step_pred, (-1, group))
                for indx, ag in enumerate(np.reshape(step_labels, (-1, group))):
                    if sum(ag) != 0:
                        group_preds.append(the_batch_group_preds[indx])
                        group_labels.append(ag)
                # group_preds.extend(np.reshape(step_pred, (-1, group)))
                # group_labels.extend(np.reshape(step_labels, (-1, group)))


        # add, filter users with no True labels
        df = pd.DataFrame({'users': users, 'preds': preds, 'labels': labels})
        # filter_count = df[["users", "labels"]].groupby("users").sum().reset_index().rename(columns={"labels": "count"})
        # filter_count = filter_count[filter_count['count'] > 0]

        def temp(rows):
            d = {'users': [rows['users'].values[0]], 'all_zero_all_one': [
                1 if rows[rows['labels'] == 0].shape[0] == 0 or rows[rows['labels'] == 0].shape[0] == rows.shape[
                    0] else 0]}
            return pd.DataFrame.from_dict(d)
        filter_count = df.groupby(["users"]).apply(temp)
        filter_count.index = filter_count.index.droplevel()
        filter_count = filter_count[filter_count['all_zero_all_one'] == 0]

        df = df.merge(filter_count["users"], on='users')




        users = df.users.values.tolist()
        preds = df.preds.values.tolist()
    ...
    def _build_embedding(self):
        ...
        with tf.variable_scope("embedding", initializer=self.initializer):
            ...
            self.looptimes_embedding_lookup = tf.get_variable( ## add
                name="looptimes_embedding",
                # shape=[10, self.item_embedding_dim + self.cate_embedding_dim],
                shape=[10, self.cate_embedding_dim],
                dtype=tf.float32,
            )
    ...
    def _lookup_from_embedding(self):
        ...
        if self.hparams.add_feature == True:
            print("add_feature")
            self.time_feature = tf.nn.embedding_lookup(  ## add
                self.looptimes_embedding_lookup,
                tf.cast(self.iterator.item_loop_times_history, dtype=tf.int32)
            )

            self.satisfied_time_feature = tf.nn.embedding_lookup( ## add
                self.looptimes_embedding_lookup,
                tf.cast(self.iterator.satisfied_play_history, dtype=tf.int32)
            )

        if self.cate_history_embedding is None:
            print(self.iterator.item_cate_history)
            exit(-1)
        ...
        # add
        self.satisfied_cate_history_embedding = tf.nn.embedding_lookup(
            self.cate_lookup, self.iterator.satisfied_cate_history
        )
        self.satisfied_item_history_embedding = tf.nn.embedding_lookup(
            self.item_lookup, self.iterator.satisfied_item_history
        )
        if self.hparams.add_feature == True:
            self.satisfied_item_history_embedding = tf.concat(
                [self.satisfied_item_history_embedding, self.satisfied_time_feature], 2
            )
            self.item_history_embedding = tf.concat(
                [self.item_history_embedding, self.time_feature], 2
            )

        tf.summary.histogram(
            "satisfied_item_history_embedding_output", self.satisfied_item_history_embedding
        )
        tf.summary.histogram(
            "satisfied_cate_history_embedding_output", self.satisfied_cate_history_embedding
        )
        ...
        # self.item_history_embedding = self.item_history_embedding
        self.satisfied_cate_history_embedding = self._dropout(
            self.satisfied_cate_history_embedding, keep_prob=self.embedding_keeps
        )
        self.satisfied_item_history_embedding = self._dropout(
            self.satisfied_item_history_embedding, keep_prob=self.embedding_keeps
        )
