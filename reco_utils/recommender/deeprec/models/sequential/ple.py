...
class PLEModel(SequentialBaseModel):
    def _ple_layer(self, x):
        ...
        share_experts_output = []
        for i in range(hparams.share_expert_num):
            share_experts = tf.expand_dims(self._fcn_transform_net(x, hparams.expert_layer_sizes, "share_expert_{}".format(i)), 1)
            share_experts_output.append(share_experts)


        task_experts_output = {}
        for task_name in ["main", "sub"]:
            experts_output = []
            for i in range(hparams.independent_expert_num):
                scope_name = "{}_expert_{}".format(task_name, i)
                experts_output.append(tf.expand_dims(self._fcn_transform_net(x, hparams.expert_layer_sizes, scope_name), 1)) # batch_size * 1 * dim
            task_experts_output[task_name] = experts_output

        res_output = {}
        for task_name in ["main", "sub"]:
            gate = tf.expand_dims(self._fcn_transform_net(x, hparams.gate_layer_sizes, "gate_{}".format(task_name)),
                                       1)  # batch_size * 1 * experts_num
            experts = tf.concat(share_experts_output + task_experts_output[task_name], 1)
            output = tf.matmul(gate, experts)
            output = tf.squeeze(output, [1])
            res_output[task_name] = output
        return res_output["main"], res_output["sub"]
    ...
    def _build_train_opt(self):
        ...
        return super(PLEModel, self)._build_train_opt()


    def _build_embedding(self):
        """The field embedding layer. Initialization of embedding variables."""
        super(PLEModel, self)._build_embedding()
        hparams = self.hparams
    ...
    def _lookup_from_embedding(self):
        ...
        """
        super(PLEModel, self)._lookup_from_embedding()
    ...
    def _build_seq_graph(self):
        ...
        with tf.variable_scope("clsr"):
            ...
            with tf.variable_scope("long_term"):
                att_outputs_long = self._attention_fcn(self.target_item_embedding, hist_input)
            ...
            model_output, self.valid_output = self._ple_layer(tf.concat([self.att_fea_long, self.att_fea_short, self.target_item_embedding], 1))
            # model_output = tf.concat([self.att_fea_long, self.target_item_embedding], 1)
            # self.valid_output = tf.concat([self.att_fea_short, self.target_item_embedding], 1)
