...
class BaseModel:
    ...
    def _compute_data_loss(self):
        if self.hparams.loss == "cross_entropy_loss":
            data_loss = tf.reduce_mean(
                ...
                # tf.losses.log_loss(
                    # predictions=tf.reshape(self.logit, [-1]),
    ...
    def _mlp_base(self, model_output, layer_sizes, scope):
        """Construct the MLP part for the model.
        ...
        with tf.variable_scope(scope):
            ...
            with tf.variable_scope("nn_part", initializer=self.initializer) as scope:
                for idx, layer_size in enumerate(layer_sizes):
                    ...
                    layer_idx += 1
                    last_layer_size = layer_size
                # self.last_gailv = hidden_nn_layers[-1]
                # self.last_emb = hidden_nn_layers[-2]
