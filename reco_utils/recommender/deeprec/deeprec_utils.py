...
def create_hparams(flags):
    ...
    init_dict = {
        ...
        "contrastive_loss": "bpr",
        "triplet_margin": 1.0,
        "discrepancy_loss_weight": 0.0,
        "contrastive_loss_weight": 0.0,
        "contrastive_length_threshold": 1,
        "contrastive_recent_k": 3,
        "reg_kg": 0.0,
        "learning_rate": 0.001,
        "lr_rs": 1,
        "lr_kg": 0.5,
        ...
        "vector_alpha": False,
        "manual_alpha": False,
        "manual_alpha_value": 0.5,
        "interest_evolve": True,
        "predict_long_short": True,
        "dtype": 32,
        "optimizer": "adam",
        "epochs": 10,
        "batch_size": 1,
        ...
        "min_seq_length": 1,
        # lightGCN
        "counterfactual_recent_k": 5,
        "use_complex_attention": False,
        "sequential_model": 'time4lstm',
        "time_unit": 's',
        "ncf_layer_sizes": [80, 40],
        # sum
        # "slots": 5,
        # "cell": "SUM",
    }
...
def cal_weighted_metric(users, preds, labels, metrics):
    ...
    for metric in metrics:
        ...
        elif metric.startswith("wmrr"): # add
            mrr_list = [10]
            ks = metric.split("@")
            if len(ks) > 1:
                mrr_list = [int(token) for token in ks[1].split(";")]
            wmrr_res = cal_wmrr_k(df, weight, mrr_list)
            res.update(wmrr_res)
...
# add
def cal_wmrr_k(df, weight, mrr_list):
    wmrr_res = {}
    weight["mrr_k"] = df.groupby("users").apply(groupby_mrr_k, mrr_list=mrr_list)
    wmrr_score = (weight["weight"] * weight["mrr_k"]).sum()
    weight.drop(columns="mrr_k", inplace=True)
    for i, k in enumerate(mrr_list):
        metric = "wmrr@{0}".format(k)
        wmrr_res[metric] = round(wmrr_score[i], 4)

    return wmrr_res

def groupby_mrr_k(df, mrr_list):

    y_hat = df.preds
    y = df.labels
    def sub_mrr(y_true, y_score, k):

        order = np.argsort(y_score)[::-1][:k]
        y_true = np.take(y_true, order)
        rr_score = y_true / (np.arange(len(y_true)) + 1)
        return np.sum(rr_score)
    return np.array([sub_mrr(y, y_hat, k) for k in mrr_list])
